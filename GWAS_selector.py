...
- subset_phenotypes.csv  (columns: sampleID, cateogrical target (poultry, ruminants, wild_birds))
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for pth in out_paths:
            ...
            if vcol is None or pcol is None:
                logger.error(
                    f"[OVR:{label}] colonne variant/pval non trovate in {pth} "
            ...
            gwas_list.append(
                df[[vcol, pcol]].rename(columns={vcol: "variant", pcol: "pval"})
            )
            del df
            del cols
            del vcol
...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if getattr(args, "use_patterns_bonferroni", False) and args.alpha_patterns is not None:
            bonf_p_thr = bonferroni_threshold_from_patterns(
                cg=cg,
        ...
        for cls in classes:
            ...
            gwas_cls = run_ovr_gwas_for_class(
                label=cls,
                y_bin=y_bin,
                cg=cg,
            ...
            per_class_stats[cls] = {
                "n_sig": n_sig,
                "n_loci_final": len(sel_loci),
            }
            logger.info(
                f"[OVR:{cls}] FINAL loci={len(sel_loci)} "
                f"(significant={n_sig}, "
        ...
        if args.multi == "union":
            union_loci = (
                sorted(set().union(*per_class_selected_loci.values()))
                if per_class_selected_loci
                else []
            )
            logger.info(f"[AGGR] union loci across classes: {len(union_loci)}")
            # Controllo interno: nessun duplicato nella union
            assert len(union_loci) == len(set(union_loci)), (
                "[AGGR] union_loci contains duplicates, which should be impossible."
            )
            # Tutti i loci devono esistere come colonne del cgMLST
            missing = [l for l in union_loci if l not in cg.columns]
            if missing:
                logger.error(
                    f"[AGGR] Some union loci not found in cgMLST columns: "
    ...
    with open(results_out, "w", encoding="utf-8") as f:
        ...
        if getattr(args, "use_patterns_bonferroni", False) and bonf_p_thr is not None:
            f.write(
                f"Bonferroni pattern-based threshold (alpha_patterns={args.alpha_patterns}): "
